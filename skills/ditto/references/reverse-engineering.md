...
Android Flutter phases require successful JADX, Apktool, and r2Flutter MCP sessions for the same original APK hash. All three exports are collected with the original pack. Missing or incompatible required capability blocks the phase.

Start with package-wide indexes, then retain only phase-relevant files and findings. The contract's `reverse_engineering.include_globs` controls packaged resources copied into `original/reverse.001/`. Each MCP export includes its active preflight receipt and bounded output. The reverse index hashes every retained file and links useful entries to checkpoints.
