...
Use the phase contract and original manifest as the replay script. `inspect_ui` returns bounded visible labels, IDs, and bounds; use `tap_target` and `wait_target` when those labels are stable. Use `observe_screen` only when an image is needed. `perform` records action arguments; label each declared action with its exact `step` text. `replay(steps=[...])` batches up to 100 actions. `run_checkpoints(plan=[...])` batches replay, an optional expected UI target, and capture per checkpoint; it stops at the first mismatch and preserves earlier completed checkpoints. Supply a concrete `observed_state` for each checkpoint and inspect the captured PNG/XML before accepting it. Setup/fixture labels describe intended preparation; they do not themselves prove it ran.

For an unfamiliar original path, try two targeted AI navigation attempts. If the branch remains unclear, start `recorder_control(operation="start", contract_path="...")` after `mobile_control(operation="begin", ...)` and give its localhost URL to the human. The human clicks or swipes the current still, types or presses Back, and captures checkpoints in order. The panel records actions through the same MCP session, names PNG/XML/trace files from the contract, and marks incidental system actions separately. This is data collection, not mid-phase human review. Stop the recorder through `recorder_control` after its Finish action before AI control resumes. Do not use a separate unrecorded ADB/manual click path. No video or paid device is required.
